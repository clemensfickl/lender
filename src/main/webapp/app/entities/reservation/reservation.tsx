    ...
    return (
      <div>
        <h2 id="reservation-heading">
          <Translate contentKey="lenderApp.reservation.home.title">Reservations</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="lenderApp.reservation.home.createLabel">Create new Reservation</Translate>
          </Link>
        </h2>
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={this.sort('start')}>
                    <Translate contentKey="lenderApp.reservation.start">Start</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={this.sort('end')}>
                    <Translate contentKey="lenderApp.reservation.end">End</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="lenderApp.reservation.borrower">Borrower</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="lenderApp.reservation.item">Item</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {reservationList.map((reservation, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>
                      <TextFormat type="date" value={reservation.start} format={APP_DATE_FORMAT} />
                    </td>
                    <td>
                      <TextFormat type="date" value={reservation.end} format={APP_DATE_FORMAT} />
                    </td>
                    <td>{reservation.borrowerLogin ? reservation.borrowerLogin : ''}</td>
                    <td>{reservation.itemName ? <Link to={`item/${reservation.itemId}`}>{reservation.itemName}</Link> : ''}</td>
